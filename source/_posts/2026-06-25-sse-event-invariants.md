...
流式体验给用户的是逐步反馈，工程上承担的却是一条分布式事件协议。只有把不变量写进解析器、存储和监控，系统才知道一段回答是真的完成，还是仅仅在 HTTP 200 之后停止了。出现断链后如何逐层找证据，见 [Agent 诊断证据链](/2026/03/25/agent-diagnostic-evidence-chain/)；协议修复后的验证方法见 [Agent 回归验证](/2025/08/04/agent-regression-fixed-environment/)。
