...
function intensity(tokens, maximum) {
    if (tokens === 0 || maximum === 0)
        return 0;
    return Math.min(4, Math.max(1, Math.ceil(Math.sqrt(tokens / maximum) * 4)));
}
/** Build Sunday-to-Saturday columns ending in the UTC week containing endDay. */
export function buildActivityWeeks(rows, endDay, weekCount = 52) {
    const end = dateOf(endDay);
    if (!Number.isFinite(end.getTime()))
        throw new Error(`invalid activity end day: ${endDay}`);
    const start = new Date(end.getTime() - (end.getUTCDay() + (weekCount - 1) * 7) * DAY_MS);
    const byDay = new Map(rows.map(row => [row.day, row]));
    let maximum = 0;
    for (const row of rows) {
        if (row.day < dayString(start) || row.day > endDay)
            continue;
