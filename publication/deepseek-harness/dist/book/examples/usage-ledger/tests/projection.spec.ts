...
describe('model cost projection', () => {
  it('publishes a wire view and validates its distinct persisted state for Harness 0.1.1', () => {
    ...
    const value = definition.wire.viewSchema.parse(definition.wire.view(state))
    expect(value).toEqual(definition.view(state))
    expect(value.currency).toBe('USD')
    expect(definition.stateSchema.safeParse(value).success).toBe(false)
    expect(definition.wire.viewSchema.safeParse(state).success).toBe(false)
  })
