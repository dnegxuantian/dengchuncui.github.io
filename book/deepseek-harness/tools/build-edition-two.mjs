...
for(const file of readdirSync(source))if(/\.(md|json|png|svg|puml)$/.test(file))copyFileSync(new URL(file,source),new URL(file,out));
for(const stem of ['02-composition','02-tool-stages'])for(const ext of ['svg','puml'])copyFileSync(new URL(`diagrams/${stem}.${ext}`,root),new URL(`${stem}.${ext}`,out));
...
const md='---\ntitle: "DeepSeek Harness：即插即用的实战指南"\nsubtitle: "第二版重写样章 · 从一份作业日报开始"\nauthor: "邓明瑞（纯粹）"\nlang: zh-CN\n---\n\n本页包含重写中的 '+chapters.length+' 章，不是完成版。\n\n'+chapters.map(f=>readFileSync(new URL(f,source),'utf8')).join('\n\n');
const complete=md+'\n\n'+readFileSync(new URL('appendix-experiments.md',source),'utf8');
execFileSync('pandoc',['--from=markdown','--standalone','--toc','--toc-depth=2','--include-in-header',new URL('tools/reader-head.inc',root).pathname,'-o',new URL('index.html',out).pathname],{input:complete});
