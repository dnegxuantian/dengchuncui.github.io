...
export function chargeNano(buckets, rate) {
    const value = Math.round((buckets.uncachedInputTokens * rate.uncachedInput
...
export function findRate(config, provider, model, time) {
    let selected;
    for (const rate of config.rates) {
        if (rate.provider !== provider || rate.model !== model || rate.effectiveFromMs > time)
            continue;
        if (selected === undefined || rate.effectiveFromMs > selected.effectiveFromMs)
            selected = rate;
    }
    return selected;
}
...
export function resolveConfig(config) {
    ...
    if (config.rates.length === 0)
        throw new Error('dsh-usage: rates must contain at least one schedule');
    const seen = new Set();
    const rates = config.rates.map((candidate, index) => {
        ...
        for (const field of ['uncachedInput', 'cacheRead', 'cacheWrite', 'output']) {
