...
import { aggregateUsage, cacheHitRate, hasCompleteCost, inputTokens, modelCalls, totalTokens } from '../src/client/usage-view.js'
...
describe('Usage page derivation', () => {
  it('uses all input but no output as the cache-hit denominator', () => {
    expect(cacheHitRate(projection())).toBe(20 / 32)
    expect(cacheHitRate(projection({ outputTokens: 5000 }))).toBe(20 / 32)
    expect(cacheHitRate(projection({ cacheReadTokens: 0 }))).toBe(0)
    expect(cacheHitRate(projection({ usageUnavailableRequests: 1 }))).toBeUndefined()
    expect(cacheHitRate(projection({ uncachedInputTokens: 0, cacheWriteTokens: 0, cacheReadTokens: 0 }))).toBeUndefined()
  })
